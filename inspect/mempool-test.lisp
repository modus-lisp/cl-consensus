...
(defun run ()
  ...
  ;; ---- mempool-on-block: drop confirmed + now-conflicting txs, keep children ----
  (multiple-value-bind (utxo h) (fund 12)
    (let ((mp (mp:make-mempool)))
      ;; A (confirmed), B (conflicts with a block tx C that double-spends B's input)
      (let ((a (build-tx (list (cons (aref h 0) 0)) (list (- +coin+ 5000))))
            (b (build-tx (list (cons (aref h 1) 0)) (list (- +coin+ 5000))))
            (c (build-tx (list (cons (aref h 1) 0)) (list (- +coin+ 7000)) :seq #xfffffffd)))
        (mp:accept-tx a utxo mp :height 200000 :mtp 1500000000)
        (mp:accept-tx b utxo mp :height 200000 :mtp 1500000000)
        (check "two txs in mempool pre-block" (= (mp:mempool-size mp) 2))
        ;; a block confirms A and C (C double-spends B's input)
        (let ((removed (mp:mempool-on-block mp (list a c))))
          (check "on-block removed A (confirmed) + B (conflict)" (= removed 2))
          (check "mempool empty after block" (= (mp:mempool-size mp) 0))))
      ;; a confirmed parent's child stays (now spends a confirmed coin)
      (let* ((p (build-tx (list (cons (aref h 2) 0)) (list (- +coin+ 5000))))
             (child (build-tx (list (cons (tx:tx-txid p) 0)) (list (- +coin+ 10000))))
             (cid (tx:txid-hex child)) (pid (tx:txid-hex p)))
        (mp:accept-tx p utxo mp :height 200000 :mtp 1500000000)
        (mp:accept-tx child utxo mp :height 200000 :mtp 1500000000)
        (mp:mempool-on-block mp (list p))
        (check "confirmed parent removed" (null (mp:mempool-get mp pid)))
