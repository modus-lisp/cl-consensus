...
(defun install-serving-handlers (peer)
  "Register the read-only serving responders on PEER (getheaders now; getdata in
   Phase 2)."
...
(defun serve-daemon (&key (listen-port (w:net-port w:*network*))
  ...
  "Long-lived network-citizen daemon: follow the header tip from an OUTBOUND peer and
   SERVE inbound getheaders.  Header-chain only (no UTXO) — runs alongside the
   keep-current UTXO writer.  Blocks forever (the accept + read loops run in threads)."
  ...
  (let ((out (p:connect-peer peer-host :start-height (c:tip-height))))
    ...
    (loop
      (sleep 60)
      (format t "~&[serve] tip ~d, ~d inbound peers~%" (c:tip-height) (%reap-inbound)) (force-output))))
